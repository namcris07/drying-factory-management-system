...
export class MqttController {
  ...
  async handleTemperatureData(@Payload() data: string, @Ctx() context: MqttContext) {
    ...
    if (!isNaN(value)) {
      await this.sensorService.processAndStoreData(deviceId, 'temperature', value);
    } else {
      this.logger.warn(`Invalid temperature format received on topic ${topic}: ${data}`);
    }
  }

  @MessagePattern('+/feeds/humidity')
  async handleHumidityData(@Payload() data: string, @Ctx() context: MqttContext) {
    ...
    if (!isNaN(value)) {
      ...
      this.logger.warn(`Invalid humidity format received on topic ${topic}: ${data}`);
    }
  }
}
