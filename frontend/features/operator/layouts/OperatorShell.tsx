...
import {
  OperatorProvider,
  useOperatorContext,
} from "@/features/operator/model/operator-context";
...
const MENU_ITEMS = [
  ...
  {
    key: "/operator/realtime",
    icon: <DesktopOutlined />,
    label: "Giám sát Buồng sấy",
  },
...
function OperatorLayoutInner({ children, user }: OperatorLayoutInnerProps) {
  ...
  const selectedKey =
    MENU_ITEMS.find((item) => pathname === item.key)?.key ||
  ...
  return (
    <Layout style={{ minHeight: "100vh" }}>
      ...
      >
        ...
        <div
          style={{
            ...
            alignItems: "center",
            gap: 10,
            padding: collapsed ? "18px 22px" : "18px 18px",
        ...
        <Menu
          ...
          items={MENU_ITEMS.map((item) => ({
            ...
            style: {
              borderRadius: 7,
              margin: "2px 8px",
              width: collapsed ? undefined : "calc(100% - 16px)",
            },
          }))}
        />

        {isMounted && !collapsed && zones.length > 0 ? (
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    color:
                      pathname === `/operator/machine/${m.id}`
                        ? "#fff"
                ...
                >
                  <div
                    style={{
                      ...
                      height: 7,
                      borderRadius: "50%",
                      background: colorMap[m.status],
                      flexShrink: 0,
                    }}
                  />
                  <span>{m.name}</span>
                  <Badge
                    count={m.status === "Error" ? "!" : 0}
                    size="small"
                    style={{ background: "#ff4d4f", marginLeft: "auto" }}
                  />
                </div>
              );
            })}
      ...
      <Layout
        style={{ marginLeft: collapsed ? 80 : 216, transition: "margin 0.2s" }}
      >
        <Header
          style={{
            background: "linear-gradient(90deg, #0a1628 0%, #003a8c 100%)",
            padding: "0 20px",
            display: "flex",
            alignItems: "center",
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
              ...
              <div style={{ lineHeight: 1.3 }}>
                ...
                <div style={{ color: "rgba(255,255,255,0.4)", fontSize: 10 }}>
                  {isMounted ? `Vận hành · ${zone}` : "Vận hành"}
                </div>
              </div>
            </div>
            <Button
